...
void loop(void) 
{
	...
	byte present = 0;
  	...
  	if ( !ds.search(addr))	// Если их нет, выводим сообщение об этом
  	{
    	...
    	ds.reset_search();
    	delay(250);
    	return;	// Снова возвращаемся в цикл
  	}
 	...
 	// Определим по фану чип по первому байту
 	// Первый байт определяет чип
  	switch (addr[0])
  	{
    	case 0x10:
      		...
      		break;
    	case 0x28:
      		Serial.println("  Chip = DS18B20");	// Это как раз наш датчик температуры
     		type_s = 0;
      		break;
    	case 0x22:
 	...
 	// Код 0x44 -- это команда выполнить температурную конверсию
  	delay(1000);     // Для того, чтобы конвертировалось, нужно подождать секунду (750 милисекунд должно хватить)
